...
case class CartRate(code: String, numericCode: Int, rate: Double = 0.01, fractionDigits: Int = 2)
...
case class Coupon(code: String,
                  @JsonSerialize(using = classOf[JodaDateTimeOptionSerializer]) @JsonDeserialize(
                      using = classOf[JodaDateTimeOptionDeserializer]) startDate: Option[DateTime] = None,
                  @JsonSerialize(using = classOf[JodaDateTimeOptionSerializer]) @JsonDeserialize(
                      using = classOf[JodaDateTimeOptionDeserializer]) endDate: Option[DateTime] = None,
                  price: Long = 0,
                  customs: Map[String, Any] = Map())
...
case class RegisteredCartItem(id: String,
                              ...
                              @JsonSerialize(using = classOf[JodaDateTimeOptionSerializer]) @JsonDeserialize(
                                  using = classOf[JodaDateTimeOptionDeserializer]) birthdate: Option[DateTime] = None,
